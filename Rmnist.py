...
def test_cdot_methods(methods, time_reg_vector, dir_path, n_samples_source=1000, n_samples_targets=50, n_samples_test=200,
                      time_length=5, max_angle=90, noise=0.1, methods_names=None, cost="seq",
    ...
    Xs, ys, Xt, yt, Xtest, ytest = load_mnist_data(dir_path, n_samples_source, n_samples_targets, n_samples_test, time_length)
    ...
    # plot_accuracies(np.arange(5), scores, methods_names=cost,
    #                 fig_name="./mfig/" + fig_name + "_accuracy.png" if fig_name is not None else None)
...
if __name__ == '__main__':
    ...
    ot_BFB = OTBFBDAClassifier(clf, reg=10, regnorm=None, it=50, epochs=1000, lr=10, verbose=True)
    ...
    time_reg_vector = [50]
    cost = ["seq", "direct"]
    final_scores = np.zeros([len(cost), 5])
    ...
    for i, c in enumerate(cost):
        ...
        for run in range(N_RUNS):
            ...
            scores = test_cdot_methods(
                ...
                plot_mapping=False,
                cost=c,
                dir_path="/Users/liuhanbing/Desktop/code/RotNIST/data/",
            )
    ...
    plot_accuracies(np.arange(5), final_scores, methods_names=cost, fig_name="./mfig/seq_avg_accuracy.png")