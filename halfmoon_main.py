...
from data import Xt_all, Xt_all_domain, load_battery_data_split, load_seq_two_moon_data_test
...
def test_cdot_methods(methods, time_reg_vector, n_samples_source, n_samples_targets,
    ...
    Xs, ys, Xt, yt, angles, Xt_all_domain, yt_all_domain, Xt_random, yt_random = load_seq_two_moon_data_test(n_samples_source, n_samples_targets, time_series, noise=0.1)

    _, _, Xtest, ytest, _, _, _, _, _ = load_seq_two_moon_data_test(0, 500, time_series, noise=0.1)
...
if __name__ == '__main__':
    ...
    for t in range(8):
        ...
        for if_sort in sort_or_not:
            ...
            for sd in range(100):
                ...
                ot_series = 18 * (np.random.choice(9, time, replace=False) + 1)
                ...
                if sample_num > 0 and time > 1:
                    unsorted_ot_series = 18 * (np.random.choice(9, sample_num, replace=False) + 1)
    ...
    dataframe.to_csv("half_{}_sametarget_ssh_{}.csv".format(sort_method, target), index=False, sep=',')    
